...
{ 
    std::lock_guard<std::mutex> lock(multiplexer_mutex);
...
int16_t QTRwithMUX::read_all(QTRSensors *sArray, int quant, bool white_line)
{
    ...
    if (!on_Line) // Se o robo esta fora da linha, retorna a direcao da ultima leitura
    {
     ...
     {
       //ESP_LOGI(GetName().c_str(), "Ultima leitura no lado direito");
       return 0;
     }
    ...
    else
    {
        ...
        //cont_leituras++;
        //ESP_LOGI(GetName().c_str(), "Posição = %d, Leitura = %d, Lido %d vezes", lastPosition, sum, cont_leituras);
        return lastPosition;
    }
}
