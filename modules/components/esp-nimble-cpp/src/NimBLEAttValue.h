...
#define CONFIG_NIMBLE_CPP_ATT_VALUE_TIMESTAMP_ENABLED 1
#endif
...
inline bool NimBLEAttValue::setValue(const uint8_t* value, uint16_t len) {
  if (len > m_attr_max_len) {
    NIMBLE_LOGE("NimBLEAttValue", "value exceeds max, len=%u, max=%u", len,
                m_attr_max_len);
    return false;
  }
...
inline NimBLEAttValue& NimBLEAttValue::append(const uint8_t* value,
                                              uint16_t len) {
  if (len < 1) {
    return *this;
  }
  ...
  if ((m_attr_len + len) > m_attr_max_len) {
    NIMBLE_LOGE("NimBLEAttValue", "val > max, len=%u, max=%u", len,
                m_attr_max_len);
    return *this;
  }
