...
dataSpec::dataSpec(std::string name, bool PID_Select) {
   ...
   * */
  GearRatio = new DataAbstract<uint16_t>(
      "GearRatio", name, 10);  // verificar depois se é o motor 10:1
  ...
  WheelDiameter = new DataAbstract<uint8_t>("WheelDiameter", name,
                                            23);  // mm  // mudar depois
  dataManager->registerParamData(WheelDiameter);

  RobotDiameter = new DataAbstract<uint8_t>("RobotDiameter", name,
                                            126);  // mm  // mudar depois
  dataManager->registerParamData(RobotDiameter);

  SensorToCenter = new DataAbstract<uint16_t>("SensorToCenter", name,
                                              131);  // mm  // mudar depois
  dataManager->registerParamData(SensorToCenter);

  RadiusSensor = new DataAbstract<uint16_t>("RadiusSensor", name,
                                            164);  // mm  // mudar depois
  dataManager->registerParamData(RadiusSensor);

  MaxAngle = new DataAbstract<float>("MaxAngle", name,
                                     13.8);  // graus  // mudar depois
  dataManager->registerParamData(MaxAngle);

  MaxAngle_Center = new DataAbstract<float>("MaxAngle_Center", name,
                                            13.8);  // graus  // mudar depois
  dataManager->registerParamData(MaxAngle_Center);

  Friction_Angle = new DataAbstract<float>("Friction_Angle", name,
                                           20);  // graus  // mudar depois
  ...
  Mass_BrushON =
      new DataAbstract<float>("Mass_Brushless_ON", name, 0);  // mudar depois
  dataManager->registerParamData(Mass_BrushON);

  Malha_Aberta =
      new DataAbstract<float>("Malha_Aberta", name, 1.0);  // mudar depois
  dataManager->registerParamData(Malha_Aberta);

  MaxSpeed = new DataAbstract<float>("MaxSpeed", name, 0);  // mudar depois
  dataManager->registerParamData(MaxSpeed);

  MetersPerPulse = new DataAbstract<float>("MetersPerPulse", name, 0);  // mudar depois
  dataManager->registerParamData(MetersPerPulse);
}