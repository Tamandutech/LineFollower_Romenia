...
dataPID::dataPID(std::string name) {  // Objeto utilizado para guardar contantes
                                      // do cálculo PID
  ...
  Kp_Speed = new DataAbstract<double>("Kp_Speed", name, 4);
  dataManager->registerParamData(Kp_Speed);
  Ki_Speed = new DataAbstract<double>("Ki_Speed", name, 4);
  dataManager->registerParamData(Ki_Speed);
  Kd_Speed = new DataAbstract<double>("Kd_Speed", name, 4);
  dataManager->registerParamData(Kd_Speed);
...
DataAbstract<float> *dataPID::getKP(
    TrackSegment track) {  // Retorna o valor de K_P para cada trecho da pista
  switch (track) {
    case LONG_LINE:
      return Kp_Long_Line;
      break;
  ...
  // ESP_LOGE(tag, "Estado do Robô ou Objeto PID inválido para esse método:
  // %s:%d para obter o Kp do PID, retornando valor null.", name.c_str(),state);
  return nullptr;
}

DataAbstract<float> *dataPID::getKD(
    TrackSegment track) {  // Retorna o valor de K_D para cada trecho da pista
  switch (track) {
    case LONG_LINE:
      return Kd_Long_Line;
      break;
  ...
  // ESP_LOGE(tag, "Estado do Robô ou Objeto PID inválido para esse método:
  // %s:%d para obter o Kd do PID, retornando valor null.", name.c_str(),state);
  return nullptr;
}
