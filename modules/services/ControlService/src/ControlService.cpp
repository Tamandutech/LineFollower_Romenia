...
ControlService::ControlService(std::string name, uint32_t stackDepth,
  ...
  // Multiplica a quantidade de revolucoes pela reducao da roda, salvando na
  // variavel MPR
  uint16_t rev =
      get_Spec->Revolution->getData();  // criada para facilitar a leitura
...
void ControlService::Run() {  // Loop do servico
  ...
  for (;;) {
    ...
    // ESP_LOGI(GetName().c_str(), "RPMService: %d",
    // eTaskGetState(this->rpm->GetHandle())); ESP_LOGI(GetName().c_str(),
    // "StatusService: %d",
    // eTaskGetState(StatusService::getInstance()->GetHandle()));
...
void ControlService::ControlePID() {
  ...
  TrackSegment RealLine_state =
      (TrackSegment)get_Status->RealTrackStatus->getData();
  ...
  if (state == CAR_STOPPED) {
    ...
    // TODO: Calcular o PID de velocidade com base no erro entre a velocidade
    // atual e a velocidade desejada
    // TODO: Adicionar Kp, Ki e Kd nas variáveis acessiveis pela dashboard
    SaveRPM();
    ...
    if (abs(PositionError) >= 6500 && OpenLoopControl &&
      ...
      if (RealLine_state == SPECIAL_TRACK) {
        ...
        PID = kpRot * erroRotation + kiRot * somaIntegradorRotacional +
              kdRot * (erroRotation - erroAnteriorRotacional);
    ...
    // ESP_LOGI(GetName().c_str(), "Erro = %.2f, VelEsq = %.2f, RPMEsq = %.2f,
    // VelDir = %.2f, RPMDir = %.2f", erro, (vel_base - PID), RPM_Left,
    // (vel_base + PID), RPM_Right);
    ...
    if (iloop > 50) {
      ...
      // ESP_LOGI(GetName().c_str(), "TargetSpeed = %d, RobotSpeed = %.2f,
      // PwmBase = %.2f", get_Speed->TargetSpeed->getData(), RobotLinearSpeed,
      // vel_base);
...
void ControlService::SaveRPM() {
  ...
  float RPM_Right = (((enc_right - lastPulseRight) / (float)MPR_Mot) /
                     ((float)deltaTimeUs / (float)60000000));
  float RPM_Left = (((enc_left - lastPulseLeft) / (float)MPR_Mot) /
                    ((float)deltaTimeUs / (float)60000000));
...
void ControlService::EnableAccelerationControlIfNeeded(float linearSpeed,
                                                       float SpeedError,
  ...
  if (abs(SpeedError) > SpeedErrorTreshold &&
      abs(PositionError) < SafePositionError)
    setAccelerationDirection(SpeedError);
}
void ControlService::DisableAccelerationWhenEnded(float SpeedError,
                                                  int16_t PositionError) {
  int16_t SafePositionError = get_Speed->SafePositionError->getData();
  if ((AccelerationStep && SpeedError < 0) ||
      abs(PositionError) >= SafePositionError)
...
float ControlService::AccelerationControl(int16_t RobotLinearSpeed,
                                          int16_t PositionError,
